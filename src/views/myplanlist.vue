            ...
            <dl class="contactus-dl">
                ...
                <dd v-for="(item, index) in list" :key="index">
                    ...
                    <div class="dl-right-con">
                        <div class="xian"></div>
                        <div class="hed">{{item.user_name}} <small>{{item.help_amount}}</small></div>
                        <div class="yue">
                        ...
                        <div class="mo">
                            <div class="yue">类型：<span>{{item.type_value}}</span></div>
                        ...
                        <router-link class="fot-change" :to="{ path: 'recharge',query:{id:item.id,card_id:item.id_card,name:encodeURIComponent(item.user_name),available_amount:item.available_amount,type:!!item.type_config ? item.type_config.id : '',text:!!item.type_config ? encodeURIComponent(item.type_config.title) : '', fr:'topbu'}}">充值<van-icon name="arrow" /></router-link>
                </dd>
            </dl>

            <router-link class="footer-button" :to="{ path: 'recharge', query: { fr: 'dibu' }}">充值</router-link>
        </div>
    <popup></popup>
    </div>
</template>
    ...
    methods:{
       firstAPi(){
           let that = this;
            ...
            axios.get(`${that.apiUrl}/getCharge`).then((res)=>{
                toast1.clear();
                that.pageshow = true;
                let {data} = res;
                if(data.status === 1){
