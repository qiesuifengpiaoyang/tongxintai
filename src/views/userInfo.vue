...
export default {
  ...
  methods: {
    ...
    firstAPi() {
      ...
      axios
        .get(`${this.apiUrl}/getCertification`)
        .then((res) => {
          ...
          if (data.status === 1) {
            ...
            if (!Array.isArray(data.info)) {
              ...
              that.card_img_front1 = httpd
                ? `${data.info.card_img_front || ""}`
                : `${that.staticUrl}/${data.info.card_img_front || ""}`;
              that.card_img_back = `${data.info.card_img_back || ""}`;
              that.card_img_back1 = httpds
                ? `${data.info.card_img_back || ""}`
                : `${that.staticUrl}/${data.info.card_img_back || ""}`;
